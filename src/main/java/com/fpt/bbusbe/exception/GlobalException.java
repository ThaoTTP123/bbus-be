...
public class GlobalException {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "409", description = "Conflict - Import lỗi",
                    content = {@Content(mediaType = APPLICATION_JSON_VALUE,
                            examples = @ExampleObject(
                                    name = "409 Import Error",
                                    summary = "Lỗi khi import file Excel có dữ liệu sai",
                                    value = """
                                        {
                                          "timestamp": "2025-03-31T09:45:00.000+00:00",
                                          "status": 409,
                                          "path": "/user/import",
                                          "error": "Conflict",
                                          ...
                                          "details": {
                                            ...
                                            "3": "Email đã tồn tại",
                                            "5": "Tên không được để trống. Số điện thoại phải có 10 chữ số"
                                          }
                                        }
                                        """
                            ))})
    })
    public ErrorResponse handleImportException(ImportException e, WebRequest request) {
        ErrorResponse errorResponse = new ErrorResponse();
        errorResponse.setTimestamp(new Date());
        errorResponse.setPath(request.getDescription(false).replace("uri=", ""));
        errorResponse.setStatus(CONFLICT.value());
        errorResponse.setError(CONFLICT.getReasonPhrase());
        errorResponse.setMessage(e.getMessage());
        errorResponse.setDetails(e.getErrorDetails()); // thêm dòng này để hiển thị chi tiết lỗi từng dòng
